...
function fnMostrarCategorias(){
	$.ajax({
		url: 'process_php/fnMostrarCategorias.php',
		type: 'POST',
		success: function(res){
			console.log(res);
			...
			for (var i = 0; i < js.length; i++) {
				row += '<div class="col-md-4 col-xs-6"><div class="shop"><div class="shop-img"><img src="'+js[i].foto+'" height="250" style="object-fit: cover;"></div><div class="shop-body"><h3>'+js[i].categoria+'</h3><a href="promociones_c.php?cat='+btoa(js[i].id_categoria)+'" class="cta-btn">ir <i class="fa fa-arrow-circle-right"></i></a></div></div></div>';
				ul += '<li id="li'+js[i].categoria+'"><a data-toggle="tab" href="#tab1" onclick="fnMostrarNuevasPromociones('+js[i].id_categoria+')">'+js[i].categoria+'</a></li>';
			}
...
function fnModalAgregarAlCarrito(id_producto,precio){
	if (id_producto==0) {
		$('#modal-login').modal('show');
	}else{
		$('#btnAgregar').attr('onclick','fnAgregarAlCarrito('+id_producto+')');
